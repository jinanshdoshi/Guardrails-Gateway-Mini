...
def analyze_request(request: AnalysisRequest):
    ...
    for doc in request.context_docs:
        clean_text, doc_pii_tags = detect_pii(doc.text)
        sanitized_docs.append({"id": doc.id, "text": clean_text})
        pii_tags.extend(doc_pii_tags) 
    injection_findings = prompt_injection(request.prompt)
    rag_findings = rag_injection([doc.model_dump() for doc in request.context_docs])
    rag_evidence = []
    rag_tags = []
